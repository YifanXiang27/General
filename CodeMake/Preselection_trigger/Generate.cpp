...
void Generate(){
    ...
    while(infile1 >> readtype){
        ...
        out_headfile<<"        "<<setw(15)<<"Bool_t"<<"ifExist"<<readdata<<";"<<endl;
        out_anaheadfile<<"        "<<setw(15)<<left<<readtype<<"R"<<readdata<<";"<<endl;
        out_anaheadfile<<"        "<<setw(15)<<"Bool_t"<<"ifExist"<<readdata<<";"<<endl;
        ...
        out_cppfile<<"    mytree->Branch(\"ifExist"<<readdata<<"\",&ifExist"<<readdata<<",\"ifExist"<<readdata;
        out_cppfile<<"/O\");"<<endl;

        out_anacppfile<<"    br = fChain->GetListOfBranches()->FindObject(\""<<readdata<<"\");"<<endl;
        out_anacppfile<<"    if(br){"<<endl;
        out_anacppfile<<"        fChain->SetBranchAddress(\""<<readdata<<"\",&R"<<readdata<<");"<<endl;
        out_anacppfile<<"        ifExist"<<readdata<<" = true;"<<endl;
        out_anacppfile<<"    }else{"<<endl;
        out_anacppfile<<"        ifExist"<<readdata<<" = false;"<<endl;
        out_anacppfile<<"    }"<<endl<<endl;

        out_analysis<<"        "<<"if(ifExist"<<readdata<<"){"<<endl;
        out_analysis<<"            "<<"myhists->"<<readdata<<" = R"<<readdata<<";"<<endl;
        out_analysis<<"            "<<"myhists->ifExist"<<readdata<<" = true;"<<endl;
        out_analysis<<"        }else{"<<endl;
        out_analysis<<"            "<<"myhists->"<<readdata<<" = false;"<<endl;
        out_analysis<<"            "<<"myhists->ifExist"<<readdata<<" = false;"<<endl;
        out_analysis<<"        }"<<endl<<endl;
